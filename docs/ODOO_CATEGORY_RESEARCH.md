...
> **Estado de implementación:** Fase 0 (badge de FieldOS) y Fase 1 completa (los 7 quick wins) ya están en el sitio. Montreal Spider Co. (`montrealspider.ca`) y Taller Arquitectura Regional (`mitchellcastellanos.github.io/Taller_Arquitectura_Regional`) ya tienen su caso completo en `portfolio.html`/`work.html` con toda la copia en EN/FR/ES — solo faltan las capturas de pantalla del dueño del negocio (las rutas de imagen ya están puestas, se ven rotas hasta que se suban los archivos a `Images/portfolio/`). `booking-system.html` (piloto de Fase 2/3) está enlazado desde `software.html` y desde el footer compartido de todo el sitio de software — sin precio fijo, todo pasa por consulta. Se agregaron además dos ofertas de upsell reales a `digital.html`: **Blog &amp; Content SEO** (enlaza al blog en vivo como prueba) y **Online Payments &amp; Deposits** — ambas sin precio, con CTA a consulta. `digital.html` también ganó un enlace de E-commerce en el footer compartido.
